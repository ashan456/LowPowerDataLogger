...
// ==== NEW: SETTIME YYYY-MM-DD HH:MM:SS | TIME?
// ===================================================
...
void runDataRetrievalMode() {
  // ==== NEW/CHANGED ====
  // Bring up I2C + RTC so SETTIME/TIME? work even in dump mode
  Wire.begin(SDA_PIN, SCL_PIN);
  ...
  Serial.println("OK v=1 caps=GET,LIST,CRC,SETTIME,TIME?"); // capability banner
  ...
  while (true) {
    ...
    if (line.startsWith("HELLO")) {
      Serial.println("OK v=1 caps=GET,LIST,CRC,SETTIME,TIME?");
    } else if (line == "LIST") {
      listDatesProtocol();
    ...
    // ==== NEW: SETTIME YYYY-MM-DD HH:MM:SS ====
      ...
      Serial.println("ERR code=UNKNOWN msg=\"use HELLO|LIST|GET YYYY-MM-DD|SETTIME YYYY-MM-DD HH:MM:SS|TIME?|BYE\"");
    }
  }
}
