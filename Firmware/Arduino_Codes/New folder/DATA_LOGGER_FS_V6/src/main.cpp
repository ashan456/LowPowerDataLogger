...
#define SCL_PIN        5
#define LED_PIN       12
#define MODE_BTN_PIN  22        // Hold LOW at boot for Dump/Data Retrieval mode
#define DEFAULT_SLEEP_SECONDS 30   // default 7.5min if no NVS value
static const char *ROOT = "/logs";

// ---- Battery sense (SET THESE) ----
#define VBAT_PIN      1
static const float VBAT_RTOP = 100000.0f;  // ohms
static const float VBAT_RBOT = 390000.0f;  // ohms
//static const float VBAT_CAL  = 1.19972f;    // tweak after DMM comparison
static const float VBAT_CAL  = 1.231f;    // tweak after DMM comparison
// ==================================================
...
void runDataRetrievalMode() {
  ...
  // Prepare VBAT ADC in server mode too
  pinMode(VBAT_PIN, INPUT);
  #if defined(ARDUINO_ARCH_ESP32)
    //analogSetPinAttenuation(VBAT_PIN, ADC_11db);
  #endif
  ...
  Serial.println("OK v=1 caps=GET,LIST,CRC,SETTIME,TIME?,SENSE?,VBAT?,INTERVAL?,SETINTERVAL"); // capability banner
  ...
  while (true) {
    ...
    if (line.startsWith("HELLO")) {
      Serial.println("OK v=1 caps=GET,LIST,CRC,SETTIME,TIME?,SENSE?,VBAT?,INTERVAL?,SETINTERVAL");
    } else if (line == "LIST") {
      listDatesProtocol();
    ...
    // ===== SETINTERVAL <seconds> -> store in NVS =====
      ...
      if (val < 30) val = 30;
      if (val > 86400UL) val = 86400UL; // clamp to 24h
      ...
      Serial.println("ERR code=UNKNOWN msg=\"use HELLO|LIST|GET YYYY-MM-DD|SETTIME YYYY-MM-DD HH:MM:SS|TIME?|SENSE?|VBAT?|INTERVAL?|SETINTERVAL|BYE\"");
    }
  }
}
...
void runLoggingMode() {
  ...
  // Load logging interval from NVS (with sane clamps)
  loadIntervalFromNVS();
