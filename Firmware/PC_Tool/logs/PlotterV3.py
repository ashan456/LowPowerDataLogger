...
(With custom color support via --colors)
"""
...
def parse_time(df: pd.DataFrame, time_col: str, date_col: Optional[str] = None) -> pd.Series:
    ...
    if date_col:
        ...
        combo = np.where(d_raw.notna(), d_raw.dt.strftime("%Y-%m-%d") + " " + t_raw, t_raw)
...
def parse_colors(arg: Optional[str]) -> Dict[str, str]:
    """
    --colors "temp=#e74c3c rh=#1f77b4 vbat=#2ecc71 soc=#f1c40f"
    """
    out = {}
    if not arg:
        return out
...
def maybe_resample(df: pd.DataFrame, rule: Optional[str]) -> pd.DataFrame:
    if not rule or "_t" not in df.columns:
        return df
...
def load_events(path: Optional[str]) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        ev["label"] = ev.get("label", ev.columns[-1]).astype(str)
...
def plot_data(df: pd.DataFrame,
    ...
    if one_figure:
        ...
        if "temp" in df_plot.columns:
            ln1, = ax.plot(df_plot["_t"], df_plot["temp"], marker=marker, linestyle="-",
                           label="Temp (°C)", color=(colors.get("temp") if colors else None))
            lns.append(ln1); labels.append("Temp (°C)")
        if "rh" in df_plot.columns:
            ln2, = ax.plot(df_plot["_t"], df_plot["rh"], marker=marker, linestyle="-",
                           label="RH (%)", color=(colors.get("rh") if colors else None))
            lns.append(ln2); labels.append("RH (%)")
        ax2 = ax.twinx()
        if "vbat" in df_plot.columns:
            unit = "V" if vbat_in_volts else "mV"
            ln3, = ax2.plot(df_plot["_t"], df_plot["vbat"], marker=marker, linestyle="-",
                            label=f"VBAT ({unit})", color=(colors.get("vbat") if colors else None))
            lns.append(ln3); labels.append(f"VBAT ({unit})")
        if soc_col:
            ln4, = ax2.plot(df_plot["_t"], df_plot[soc_col], linestyle="--",
                            label="SOC (%)", color=(colors.get("soc") if colors else None))
        ...
        if "temp" in df_plot.columns:
            ...
            ax.plot(df_plot["_t"], df_plot["temp"], marker=marker, linestyle="-",
                    color=(colors.get("temp") if colors else None))
        ...
        if "rh" in df_plot.columns:
            ...
            ax.plot(df_plot["_t"], df_plot["rh"], marker=marker, linestyle="-",
                    color=(colors.get("rh") if colors else None))
            ax.set_ylabel("RH (%)")
        ...
        if "vbat" in df_plot.columns:
            ...
            ax.plot(df_plot["_t"], df_plot["vbat"], marker=marker, linestyle="-",
                    color=(colors.get("vbat") if colors else None))
...
def main(argv=None):
    ...
    ap.add_argument("--colors", help="Series colors: 'temp=#e74c3c rh=#1f77b4 vbat=#2ecc71 soc=#f1c40f'")
    ...
    title = args.title or (f"Logger Plot — {', '.join([f.name for f in files])}")
