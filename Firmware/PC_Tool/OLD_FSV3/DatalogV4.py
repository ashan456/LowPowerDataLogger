...
# -------- NEW: RTC & SENSE helpers --------
STAMP_RE = re.compile(r'^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}$')
SENSE_RE = re.compile(r'^OK\s+T=([+-]?\d+(?:\.\d+)?)C\s+RH=([+-]?\d+(?:\.\d+)?)%$', re.IGNORECASE)
...
def query_time(ser: serial.Serial) -> str:
    ...
    # m.group(1) holds "YYYY-MM-DD HH:MM:SS"
...
def normalize_set_stamp(arg: Optional[str]) -> str:
    """Return 'YYYY-MM-DD HH:MM:SS' based on:
       - None or 'now' -> PC local time
       - explicit stamp -> validate format
    """
...
def query_sense(ser: serial.Serial) -> Tuple[float, float]:
    """Return (temp_C, rh_pct) from SENSE?"""
    send_line(ser, "SENSE?")
    line = read_line(ser, 2.0)
    if not line:
...
def main():
    ap = argparse.ArgumentParser(description="Fetch ESP32 CSV logs / manage RTC / sensor test over serial.")
    ...
    # New sensor test
    ap.add_argument("--sense", action="store_true", help="Query temperature/humidity (SENSE?) and print 'tempC,humidity%'")

    args = ap.parse_args()

    # Require at least one action
    if not (args.list or args.date or args.time or (args.settime is not None) or args.sense):
        ap.error("Provide one of: --list | --date YYYY-MM-DD | --time | --settime [YYYY-MM-DD HH:MM:SS|now] | --sense")
    ...
    try:
        ...
        if args.sense:
            t, rh = query_sense(ser)
            # CSV-friendly output: tempC,humidity%
            print(f"{t:.2f},{rh:.2f}")
