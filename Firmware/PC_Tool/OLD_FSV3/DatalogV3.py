...
def query_time(ser: serial.Serial) -> str:
    ...
    if not STAMP_RE.match(val):
        # Some firmwares may print the stamp directly after OK without group;
        # fall back to whole line after 'OK '
...
def main():
    ap = argparse.ArgumentParser(description="Fetch ESP32 CSV logs / manage RTC over serial.")
    ...
    if not (args.list or args.date or args.time or (args.settime is not None)):
        ap.error("Provide one of: --list | --date YYYY-MM-DD | --time | --settime [YYYY-MM-DD HH:MM:SS|now]")
