...
# ---- Decoder constants (must match firmware) ----
_RECORD_SIZE = 7        # BinRec size
_VBAT_MIN_MV = 2500     # enc_vbat lower bound
_VBAT_MAX_MV = 4500     # enc_vbat upper bound
...
REBOOT_RE = re.compile(r'^OK\s+REBOOT\s+REASON=(.+)$', re.IGNORECASE)
FORMAT_RE = re.compile(r'^OK\s+FORMAT\s+DONE\s+TOTAL=(\d+)\s+USED=(\d+)\s+FREE=(\d+)$', re.IGNORECASE)
DATE_INFO_RE = re.compile(r'^DATE=([^\s]+)\s+SIZE_KB=(\d+)\s+ROWS=(\d+)$')
HEARTBEAT_RE = re.compile(r'^OK\s+HEARTBEAT\s+ON=([01])\s+PERIOD=(\d+)$', re.IGNORECASE)
HEARTBEAT_PERIOD_RE = re.compile(r'^OK\s+HEARTBEAT\s+PERIOD=(\d+)$', re.IGNORECASE)
...
def decode_bin_to_csv(bin_path: Path, csv_path: Path) -> dict:
    """
    Decode firmware 7-byte records (NEW format) into CSV with columns:
      time,temperature,humidity,vbat_mV

    New 7-byte layout (little-endian):
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        ...
        for _ in range(nrec):
            ...
            # vbat (uint8 -> mV)
            vb = data[off + 6]
            vbat_mv = _VBAT_MIN_MV + round((vb / 255.0) * (_VBAT_MAX_MV - _VBAT_MIN_MV))
...
def build_log_paths(base_dir: Path, date_str: str) -> tuple[Path, Path]:
    """
    Given YYYY-MM-DD, return (bin_path, csv_path):
      bin: base_dir / f"{date}.bin"
      csv: base_dir / "decoded" / f"{date}.csv"
    """
    # Ensure the date shape looks right (defensive)
    if len(date_str) != 10 or date_str[4] != '-' or date_str[7] != '-':
        raise ValueError("date must be YYYY-MM-DD")
    bin_path = base_dir / f"{date_str}.bin"
    csv_path = base_dir / "decoded" / f"{date_str}.csv"
...
def get_bin_file(ser: serial.Serial, date: str, outpath: str, progress: bool):
    """Download a specific date's BIN file"""
    send_line(ser, f"GET {date}")
    line = read_line(ser, 3.0)
    if not line:
        raise RuntimeError("No response to GETBIN")
    ...
    # Optional CRC32 line (same as CSV flow)
    line = read_line(ser, 1.0)
    ...
    # --- NEW: decode into log/decoded/<date>.csv using the helpers you added ---
    try:
        from pathlib import Path

        bin_path = Path(outpath)
        # Build decoded CSV path in a sibling "decoded" folder, with same date-based name
        decoded_csv = bin_path.parent / "decoded" / (bin_path.stem + ".csv")

        stats = decode_bin_to_csv(bin_path, decoded_csv)
        print(f"[ok] Decoded → {decoded_csv} "
              f"(records={stats['records']}, bytes={stats['bytes']}, tail={stats['tail_bytes']})")
    except Exception as e:
        print(f"[warn] BIN downloaded but decode failed: {e}")
...
# ============================================================================
# HEARTBEAT LED COMMANDS
# ============================================================================
def query_heartbeat(ser: serial.Serial) -> Tuple[bool, int]:
    """Query heartbeat LED status and period"""
    send_line(ser, "HEARTBEAT?")
    line = read_line(ser, 2.0)
    if not line:
        raise RuntimeError("HEARTBEAT?: No response")
    if line.upper().startswith("ERR"):
        raise RuntimeError(line)
    
    m = HEARTBEAT_RE.match(line.strip())
    if not m:
        raise RuntimeError(f"HEARTBEAT?: Unexpected: {line}")
    
    on = bool(int(m.group(1)))
    period = int(m.group(2))
    return on, period

def set_heartbeat_on(ser: serial.Serial):
    """Enable heartbeat LED"""
    send_line(ser, "HEARTBEAT ON")
    line = read_line(ser, 2.0)
    if not line:
        raise RuntimeError("HEARTBEAT ON: No response")
    if line.upper().startswith("ERR"):
        raise RuntimeError(line)
    if line.strip() != "OK HEARTBEAT ON":
        raise RuntimeError(f"HEARTBEAT ON: Unexpected: {line}")

def set_heartbeat_off(ser: serial.Serial):
    """Disable heartbeat LED"""
    send_line(ser, "HEARTBEAT OFF")
    line = read_line(ser, 2.0)
    if not line:
        raise RuntimeError("HEARTBEAT OFF: No response")
    if line.upper().startswith("ERR"):
        raise RuntimeError(line)
    if line.strip() != "OK HEARTBEAT OFF":
        raise RuntimeError(f"HEARTBEAT OFF: Unexpected: {line}")

def set_heartbeat_period(ser: serial.Serial, seconds: int) -> int:
    """Set heartbeat LED period in seconds"""
    send_line(ser, f"SETHEARTBEAT {int(seconds)}")
    line = read_line(ser, 2.0)
    if not line:
        raise RuntimeError("SETHEARTBEAT: No response")
    if line.upper().startswith("ERR"):
        raise RuntimeError(line)
    
    m = HEARTBEAT_PERIOD_RE.match(line.strip())
    if not m:
        raise RuntimeError(f"SETHEARTBEAT: Unexpected: {line}")
    return int(m.group(1))
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="ESP32 DataLogger CLI - Complete serial control interface (BIN-only downloads)",
...
Examples:
  ...
  # Download specific date (BIN)
  %(prog)s --date 2025-10-07 --out ./data/log.bin
  ...
  # Check heartbeat LED status
  %(prog)s --heartbeat
  
  # Enable heartbeat LED
  %(prog)s --heartbeat-on
  
  # Set heartbeat period to 10 seconds
  %(prog)s --setheartbeat 10
...
Environment Variables:
  ...
  LOGGER_BAUD   - Default baud rate (default: 115200)
    ...
    data.add_argument("--date", metavar="YYYY-MM-DD",
                      help="Download BIN for specific date")
    data.add_argument("--out", metavar="PATH",
                      help="Output BIN path (default: ./logs/<date>.bin)")
    ...
    # LED Heartbeat
    led = ap.add_argument_group('LED Heartbeat Control')
    led.add_argument("--heartbeat", action="store_true",
                     help="Query heartbeat LED status and period")
    led.add_argument("--heartbeat-on", action="store_true",
                     help="Enable heartbeat LED")
    led.add_argument("--heartbeat-off", action="store_true",
                     help="Disable heartbeat LED")
    led.add_argument("--setheartbeat", type=int, metavar="SECONDS",
                     help="Set heartbeat LED period in seconds")
    ...
    if not any([
        ...
        args.heartbeat, args.heartbeat_on, args.heartbeat_off, 
        args.setheartbeat is not None
    ]):
    ...
    try:
        ...
        # Heartbeat LED commands
        if args.heartbeat:
            on, period = query_heartbeat(ser)
            status = "ENABLED" if on else "DISABLED"
            print(f"Heartbeat LED: {status}")
            print(f"Heartbeat period: {period} seconds")

        if args.heartbeat_on:
            set_heartbeat_on(ser)
            print("✓ Heartbeat LED enabled")

        if args.heartbeat_off:
            set_heartbeat_off(ser)
            print("✓ Heartbeat LED disabled")

        if args.setheartbeat is not None:
            period = set_heartbeat_period(ser, args.setheartbeat)
            print(f"✓ Heartbeat period set to: {period} seconds")
        ...
        if args.date:
            outpath = args.out or os.path.join(".", "logs", f"{args.date}.bin")
            print(f"Downloading {args.date} (BIN)...", file=sys.stderr)
            get_bin_file(ser, args.date, outpath, progress=not args.no_progress)
