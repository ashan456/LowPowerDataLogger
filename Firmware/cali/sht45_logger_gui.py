# sht45_logger_gui.py
# Tkinter GUI to read "DATA,<millis>,<temp_C>,<rh_%>" from your sketch.
# Adds: Single Read button that logs exactly one sample to CSV.
...
class SHT45GUI:
    def __init__(self, root):
        ...
        self.refresh_btn = ttk.Button(frm_top, text="Refresh", command=self.refresh_ports)
        ...
        # --- Middle frame: controls & display ---
        frm_mid = ttk.Frame(root, padding=(8, 0, 8, 8))
        frm_mid.grid(row=1, column=0, sticky="ew")
        frm_mid.columnconfigure(6, weight=1)

        ttk.Label(frm_mid, text="Interval (ms):").grid(row=0, column=0, sticky="w")
        self.interval_var = tk.StringVar(value="5000")
        self.interval_entry = ttk.Entry(frm_mid, width=10, textvariable=self.interval_var)
        self.interval_entry.grid(row=0, column=1, sticky="w", padx=(4, 16))
        ...
        # NEW: Single Read button (one-shot read -> append to CSV)
        self.single_btn = ttk.Button(frm_mid, text="Single Read", command=self.single_read, state="disabled")
        self.single_btn.grid(row=0, column=4, padx=4)

        # Live readouts
        box = ttk.LabelFrame(frm_mid, text="Latest Reading", padding=8)
        box.grid(row=1, column=0, columnspan=7, sticky="ew", pady=(8, 0))
        for i in range(6):
            box.columnconfigure(i, weight=1)

        ttk.Label(box, text="System Time:").grid(row=0, column=0, sticky="e")
        self.lbl_time = ttk.Label(box, text="—")
        self.lbl_time.grid(row=0, column=1, sticky="w")

        ttk.Label(box, text="Temp (°C):").grid(row=0, column=2, sticky="e")
        self.lbl_temp = ttk.Label(box, text="—")
        self.lbl_temp.grid(row=0, column=3, sticky="w")

        ttk.Label(box, text="Humidity (%RH):").grid(row=0, column=4, sticky="e")
        self.lbl_rh = ttk.Label(box, text="—")
        self.lbl_rh.grid(row=0, column=5, sticky="w")
    ...
    def connect(self):
        ...
        try:
            ...
            self.stop_btn.config(state="disabled")
            self.single_btn.config(state="normal")  # enable Single Read when connected
                ...
                if self.ser and self.ser.is_open:
                    ...
                    if chunk:
                        ...
                        while b"\n" in buf:
                            ...
                            try:
                                s = line.decode("utf-8", errors="ignore").strip("\r")
                            except Exception:
                                s = ""
                            if s:
    ...
    # NEW: single one-shot read and append to CSV
        ...
        if self.current_csv_date != today or not self.csv_path:
            ...
            if not os.path.exists(self.csv_path):
                with open(self.csv_path, "w", newline="", encoding="utf-8") as f:
                    csv.writer(f).writerow(["timestamp", "temp_C", "rh_pct"])
        return self.csv_path

    def append_row(self, ts_str, t_c, rh):
        path = self.ensure_csv()
        try:
            with open(path, "a", newline="", encoding="utf-8") as f:
                csv.writer(f).writerow([ts_str, f"{t_c:.2f}", f"{rh:.2f}"])
        except Exception as e:
        ...
        if s.startswith("DATA,"):
            ...
            if len(parts) >= 4:
                try:
                    t_c = float(parts[2])
                    rh = float(parts[3])
                    ts = datetime.now().strftime("%H:%M:%S")   # or with date if you prefer
                    self.lbl_time.config(text=ts)
                    self.lbl_temp.config(text=f"{t_c:.2f}")
                    self.lbl_rh.config(text=f"{rh:.2f}")
                    self.append_row(ts, t_c, rh)
                    # keep the status friendly during single-shot or stream
                    if self.streaming:
                        self.status.set("Streaming…")
                    else:
                        self.status.set("Logged one sample.")
                except ValueError:
                    pass
        else:
            # Show short status like OK,READY or ERR,…
