# sht45_calibration_display_v5_confirm.py
# Compact fixed-size layout + confirmation on close/disconnect.
# Periodic read every 5s (CSV logs: TIME,TEMP(C),RH (%))
...
class CalibGUI:
    def __init__(self, root):
        ...
        self.value_font = ("Consolas", 32, "bold")
        ...
        # --- Top Bar ---
        top = ttk.Frame(root, padding=(8, 4, 8, 2))
        top.grid(row=0, column=0, sticky="ew")
        for c in range(6): top.columnconfigure(c, weight=0)
        top.columnconfigure(6, weight=1)

        ttk.Label(top, text="Port:", font=("Segoe UI", 9, "bold")).grid(row=0, column=0, sticky="w", padx=(2, 4))
        self.port_combo = ttk.Combobox(top, width=10, state="readonly", values=self.list_ports())
        ...
        self.disconnect_btn = ttk.Button(top, text="Disconnect", command=self.confirm_and_disconnect, state="disabled")
        self.disconnect_btn.grid(row=0, column=4, sticky="w")

        # --- Controls (Read Once / Periodic) ---
        ...
        self.read_once_btn = tk.Button(
            controls, text="Read Once", command=self.sense_once,
            font=self.button_font, relief="raised", height=1
        )
        self.read_once_btn.grid(row=0, column=0, sticky="ew")

        ttk.Frame(controls, width=12).grid(row=0, column=1)  # spacer

        self.periodic_btn = tk.Button(
            controls, text="Start Periodic Readings", command=self.toggle_periodic,
            font=self.button_font, relief="raised", height=1,
        ...
        # --- Readouts ---
        panel = ttk.Frame(root, padding=(8, 4, 8, 2))
        ...
        ttk.Label(root, textvariable=self.status, anchor="w", padding=(8, 2)).grid(row=3, column=0, sticky="ew")

        # Loop + close hook
        root.after(100, self.poll_queue)
        root.protocol("WM_DELETE_WINDOW", self.confirm_and_exit)
    ...
    def append_csv_row(self, ts, temp_c, rh):
    ...
    def confirm_and_disconnect(self):
        if not (self.ser and self.ser.is_open):
            ...
            return
        msg = "Disconnect from the device?"
        if self.periodic_active:
            msg = "Periodic reading is active.\n\nDisconnect from the device?"
        if not messagebox.askyesno("Confirm Disconnect", msg):
            return
        self.disconnect()
    ...
    def toggle_periodic(self):
        if not self.periodic_active:
            ...
            self.periodic_btn.config(text="Stop Periodic Readings", bg="#3CB371", activebackground="#2E8B57")
    ...
    def confirm_and_exit(self):
        ...
        msg_lines = ["Exit the application?"]
        if self.ser and self.ser.is_open:
            msg_lines.append("The device will be disconnected.")
        if self.periodic_active:
            msg_lines.append("Periodic reading is currently active.")
