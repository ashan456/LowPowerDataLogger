# sht45_logger_gui.py
# Simple Tkinter GUI to read "DATA,<millis>,<temp_C>,<rh_%>" lines from your firmware
# and append "timestamp,temp_C,rh_pct" to a daily CSV.
...
class SHT45GUI:
    def __init__(self, root):
        ...
        frm_mid.columnconfigure(4, weight=1)
        ...
        box.grid(row=1, column=0, columnspan=5, sticky="ew", pady=(8, 0))
        for i in range(6):
        ...
        if self.current_csv_date != today or not self.csv_path:
            ...
            if new_file:
                with open(self.csv_path, "w", newline="", encoding="utf-8") as f:
                    w = csv.writer(f)
                    w.writerow(["timestamp", "temp_C", "rh_pct"])
    ...
    def append_row(self, ts_str, t_c, rh):
        ...
        try:
            with open(path, "a", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow([ts_str, f"{t_c:.2f}", f"{rh:.2f}"])
        except Exception as e:
    ...
    def handle_serial_line(self, s: str):
        # Expected "DATA,<millis>,<temp>,<rh>" from the sketch
        # We also pass through other lines to status.
        if s.startswith("DATA,"):
            parts = s.split(",")
            if len(parts) >= 4:
                try:
                    # t_ms = int(parts[1])  # not used for CSV
                    t_c = float(parts[2])
                    rh = float(parts[3])
            ...
            # Show short status lines like OK,READY / ERR,…
...
def main():
    root = tk.Tk()
    # Platform-friendly default theme
    try:
